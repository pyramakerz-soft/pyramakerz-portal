        ...
        <div class="container-fluid full__width__padding">
            <div class="row">
                ...
                <div class="col-xl-4 col-lg-12 col-md-12 col-sm-12 col-12">
                    <div class="accordion content__cirriculum__wrap" id="lessonHierarchyAccordion">
                        @php
                        $groupedLessons = $lessons->groupBy('course_path_id');
                        @endphp

                        @foreach ($groupedLessons as $coursePathId => $coursePathLessons)
                        <div class="accordion-item mb-3">
                            <h2 class="accordion-header" id="coursePathHeading{{ $coursePathId }}">
                                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse"
                                    data-bs-target="#coursePathCollapse{{ $coursePathId }}"
                                    aria-expanded="false"
                                    aria-controls="coursePathCollapse{{ $coursePathId }}">
                                    {{ $coursePathLessons->first()['course_path_name'] ?? 'Unnamed Course Path' }}
                                </button>
                            </h2>
                            <div id="coursePathCollapse{{ $coursePathId }}" class="accordion-collapse collapse"
                                aria-labelledby="coursePathHeading{{ $coursePathId }}"
                                data-bs-parent="#lessonHierarchyAccordion">
                                <div class="accordion-body">

                                    @php
                                    $pathsGrouped = $coursePathLessons->groupBy('path_id');
                                    @endphp

                                    <div class="accordion" id="pathAccordion{{ $coursePathId }}">
                                        @foreach ($pathsGrouped as $pathId => $pathLessons)
                                        <div class="accordion-item mb-2">
                                            <h2 class="accordion-header" id="pathHeading{{ $pathId }}">
                                                <button class="accordion-button collapsed" type="button"
                                                    data-bs-toggle="collapse"
                                                    data-bs-target="#pathCollapse{{ $pathId }}"
                                                    aria-expanded="false"
                                                    aria-controls="pathCollapse{{ $pathId }}">
                                                    {{ $pathLessons->first()['path_name'] ?? 'Unnamed Path' }}
                                                </button>
                                            </h2>
                                            <div id="pathCollapse{{ $pathId }}" class="accordion-collapse collapse"
                                                aria-labelledby="pathHeading{{ $pathId }}"
                                                data-bs-parent="#pathAccordion{{ $coursePathId }}">
                                                <div class="accordion-body">

                                                    @foreach ($pathLessons as $index => $lesson)
                                                    @php
                                                    ...
                                                    <div class="dashboard__single__counter lesson-card"
                                                        ...
                                                        @if($meeting && $isSessionCompleted)
                                                        <a href="{{ route('meetings.evaluate', $meeting->id) }}" class="btn btn-sm default__button">Evaluate Session</a>
                                                        @endif
