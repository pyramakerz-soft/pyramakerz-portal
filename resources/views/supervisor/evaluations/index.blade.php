                        ...
                        <div class="dashboard__content__wraper">
                            ...
                            <form action="{{ route('admin.evaluations.index') }}" method="GET" class="mb-4">
                                <div class="row">
                                    ...
                                    <div class="col-md-3">
                                        <input type="number" name="min_percentage" class="form-control" placeholder="Min %" value="{{ request('min_percentage') }}">
                                    </div>
                            ...
                            <div class="table-responsive">
                                <table class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="8" class="text-center">No Auto Evaluations Found.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
                                    ...
                                    <tbody>
                                        @forelse ($manualEvaluations as $manualEvaluation)
                                            <tr>
                                                ...
                                                <td>{{ \Carbon\Carbon::parse($manualEvaluation->date)->format('Y-m-d') }}</td>
                                                <td>{{ $manualEvaluation->program }}</td>
                                                <td>{{ $manualEvaluation->rank }}</td>
                                                <td>{{ number_format($manualEvaluation->evaluation_percentage, 2) }}%</td>
                                                <td>{{ $manualEvaluation->supervisor->name }}</td>
                                                <td>
                                                    <button class="btn btn-sm btn-warning view-comments-btn" data-id="{{ $manualEvaluation->id }}">
                                        ...
                                        @empty
                                            <tr>
                                                <td colspan="7" class="text-center">No Manual Evaluations Found.</td>
                                            </tr>
                                        @endforelse
                                    </tbody>
                                </table>
...
<script>
    $(document).ready(function () {
        $(".view-comments-btn").click(function () {
            ...
            $.get("{{ url('/admin/evaluations/manual/comments') }}/" + evaluationId, function (comments) {
