...
class LessonController extends Controller
{
    ...
    public function uploadResource(Request $request)
    {
        $request->validate([
            ...
            'resources.*.type' => 'required|in:session,teaching_guide,project,quiz,assignment,handout',
            'resources.*.uploadType' => 'required|in:file,link',
            'resources.*.file' => 'nullable|file|mimes:pdf,doc,docx,ppt,pptx,zip,rar|max:10240',
            'resources.*.link' => 'nullable|url|max:500',
        ]);
        ...
        foreach ($request->resources as $index => $resource) {
            ...
            if ($resource['uploadType'] === 'file' && isset($resource['file'])) {
                $file = $resource['file'];
                $originalExtension = $file->getClientOriginalExtension();
                $originalName = pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
                $timestamp = time();

                if ($resource['type'] === 'handout' && $originalExtension === 'zip') {
                    ...
                    if ($zip->open($zipPath) === true) {
                        ...
                        return response()->json(['message' => "Failed to extract zip file for resource #{$index}"], 500);
                    }
                } else {
                    $imageName = $timestamp . '_' . uniqid() . '.' . $originalExtension;
                    $file->move(public_path('lesson_materials'), $imageName);
                    $filePath = 'lesson_materials/' . $imageName;
                }
            }

            $visibility = match ($resource['type']) {
            ...
            \App\Models\LessonResource::create([
                ...
                'title'         => $resource['type'], // Default title = type, you can enhance this
                'description'   => null, // Add dynamic description if needed
                'file_path'     => $filePath,
                'resource_link' => $resource['uploadType'] === 'link' ? $resource['link'] : null,
